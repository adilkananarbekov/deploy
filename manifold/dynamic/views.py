...
def manifold(request):
    ...
    card = CardSlider.objects.order_by('-id')[:3]
    if request.method == 'POST':
        email = request.POST.get('email')
        user = Subscribers(email = email)
        user.save()
    return render(request, 'dynamic/footer.html', {'logo':logo, 'testimonials':testimonials, 'brands':brands, 'socials':social, 'help_list':help_list, 'care_list':care, 'cards':card})
